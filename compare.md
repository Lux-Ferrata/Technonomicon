...
| `hyprland` input | `github:hyprwm/Hyprland` — bleeding-edge flake | Uses nixpkgs `programs.hyprland` (stable release) | PARTIAL | `[x]` |
| `nix-colors` input | `github:misterio77/nix-colors` — base16 color scheme library | Not present; colors are hardcoded Nord | NEW | `[x]` |
...
|---|---|---|---|---|
| `omarchy.full_name` | User's full name (string) | Hardcoded `"xin"` in shell.nix git config | NEW (pattern) | `[x]` |
| `omarchy.email_address` | User's email (string) | Hardcoded `"git@ironshark.org"` in shell.nix | NEW (pattern) | `[x]` |
| `omarchy.theme` | Enum: tokyo-night, kanagawa, everforest, catppuccin, nord, gruvbox, gruvbox-light, generated_light, generated_dark; default tokyo-night | No theme system; Nord hardcoded everywhere | NEW | `[x]` |
| `omarchy.theme_overrides.wallpaper_path` | Path to custom wallpaper; null by default | No wallpaper at all | NEW | `[x]` |
| `omarchy.primary_font` | String; default `"Liberation Sans 11"` | Fonts set per-app; Iosevka, JetBrains Mono, etc. | PARTIAL | `[x]` |
| `omarchy.vscode_settings` | Attrs passed to VS Code user settings | VSCodium settings hardcoded in neovim.nix | PARTIAL | `[ ]` |
| `omarchy.monitors` | List of Hyprland monitor strings | Single monitor rule `output=""` auto in hyprland.lua | PARTIAL | `[x]` |
| `omarchy.scale` | Int display scale factor (1 or 2); default 2 | `xwayland.force_zero_scaling = true`, no scale option | NEW | `[x]` |
| `omarchy.quick_app_bindings` | List of Hyprland bind strings for app shortcuts; default includes ChatGPT, calendar, email, YouTube, WhatsApp, X | Hardcoded keybindings in hyprland.lua | PARTIAL | `[x]` |
...
| `nord` | `nord` | `Nord` | ALREADY PRESENT (Nord already used everywhere) | `[x]` |
| `gruvbox` | `gruvbox-dark-hard` | `Gruvbox Dark Hard` | NEW | `[ ]` |
| `gruvbox-light` | `gruvbox-light-medium` | `Gruvbox Light Medium` | NEW | `[ ]` |
| `custom` | (dynamic generation) | `Tokyo Night` fallback | NEW | `[x]` |
...
| `hyprpicker` | Color picker under cursor, copies to clipboard | Has `wl-color-picker` (art.nix) | PARTIAL | `[x]` |
| `hyprsunset` | Blue light filter (reduces blue light at night) | Not present | NEW | `[x]` |
...
| `libnotify` | `notify-send` for desktop notifications | Used in shell scripts; not explicitly in packages | NEW | `[x]` |
...
| `clipse` | TUI clipboard manager with history, persists clipboard after app close | Has `copyq` (hyprland.nix) | CONFLICT | `[x]` |
...
| `lazydocker` | TUI Docker manager | Not present | NEW | `[x]` |
| `btop` | TUI system monitor | Present in shell.nix | ALREADY PRESENT | — |
| `powertop` | CPU/power usage analyzer (Intel) | Not present | NEW | `[x]` |
...
| `github-desktop` | GUI GitHub client | Has `gh` CLI + `lazyjj` | PARTIAL | `[x]` |
| `gh` | GitHub CLI | Present in shell.nix | ALREADY PRESENT | — |
| `docker-compose` | Multi-container Docker orchestration | Has `docker` but not compose | NEW | `[x]` |
| `ffmpeg` | Video/audio processing | Present in art.nix | ALREADY PRESENT | — |
| `typora` | Markdown editor with WYSIWYG preview (x86_64 only, unfree) | Not present | NEW | `[x]` |
...
| Feature | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `security.rtkit.enable = true` | Real-time kit for audio priority | Not set (PipeWire works without it but rtkit is recommended) | NEW | `[x]` |
| `services.pipewire.jack.enable = true` | JACK audio support via PipeWire | Only `alsa` + `pulse` enabled in desktop.nix | NEW | `[x]` |
...
| `fonts.packages`: `noto-fonts-color-emoji` | Color emoji font | Not explicitly present | NEW | `[x]` |
...
| `programs.hyprland.package` | From `inputs.hyprland` flake | Default nixpkgs package | PARTIAL | `[x]` |
...
| Feature | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `virtualisation.containers.enable = true` | OCI container support | Not explicitly set | NEW | `[x]` |
...
| Feature | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `nix-colors.homeManagerModules.default` import | Provides `colorScheme` option and base16 palette | Not present | NEW | `[x]` |
| `colorScheme = inputs.nix-colors.colorSchemes.${selectedTheme.base16-theme}` | Sets active color scheme from theme name | Not present; colors hardcoded per app | NEW | `[x]` |
| Generated color scheme via `colorSchemeFromPicture` | Extracts base16 colors from wallpaper image | Not present | NEW | `[x]` |
| `gtk.theme.name = "Adwaita:dark"` (or `"Adwaita"` for light) | GTK theme tied to light/dark theme selection | Hardcoded `"Adwaita-dark"` in desktop.nix | PARTIAL | `[x]` |
...
| `home.file.".local/share/omarchy/bin"` | Copies bundled scripts to `~/.local/share/omarchy/bin` | Scripts deployed to `/etc/scripts/` | PARTIAL | `[x]` |
...
| Feature | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `wayland.windowManager.hyprland.enable = true` | Enables Hyprland HM module | Technonomicon configures Hyprland via `home.file` (raw config files) | PARTIAL | `[x]` |
| `wayland.windowManager.hyprland.package` | From `inputs.hyprland` flake | Not set (uses system package) | PARTIAL | `[x]` |
| `services.hyprpolkitagent.enable = true` | Hyprland's built-in polkit authentication agent | Not present; no polkit agent running | NEW | `[x]` |
...
| Monitor config from `cfg.monitors` | Passed in from omarchy options | Single `output="" mode=preferred position=auto scale=auto` | PARTIAL | `[x]` |
...
|---|---|---|---|---|
| `hyprsunset` | Blue light filter daemon | Not present | NEW | `[x]` |
| `systemctl --user start hyprpolkitagent` | Polkit agent for privilege escalation dialogs | Not present | NEW | `[x]` |
| `wl-clip-persist --clipboard regular` | Keeps clipboard content alive after app closes | Not present | NEW | `[x]` |
| `clipse -listen` | Background clipboard history daemon | Uses `copyq --start-server` | CONFLICT | `[x]` |
...
| `SUPER + V` | `togglefloating` | Not present | NEW | `[x]` |
| `SUPER SHIFT + Plus` | `fullscreen` | Not present | NEW | `[x]` |
...
| `SUPER + K` | `omarchy-show-keybindings` script → wofi | Not present | NEW | `[x]` |
...
| `SUPER + comma` | Switch to previous workspace | Not present | NEW | `[x]` |
| `SUPER + period` | Switch to next workspace | Not present | NEW | `[x]` |
...
| `SUPER + minus` | `resizeactive -100 0` (shrink width) | Not present | NEW | `[x]` |
| `SUPER + equal` | `resizeactive 100 0` (grow width) | Not present | NEW | `[x]` |
| `SUPER SHIFT + minus` | `resizeactive 0 -100` (shrink height) | Not present | NEW | `[x]` |
...
| Binding | omarchy-nix action | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `CTRL SUPER + V` | `ghostty --class clipse -e clipse` (clipboard TUI) | Not present (copyq has its own UI) | NEW | `[x]` |
...
| `GDK_SCALE,${cfg.scale}` | Display scale from config option | Not set (uses `QT_SCALE_FACTOR=1`, `GDK_SCALE=1`) | PARTIAL | `[x]` |
...
| `QT_STYLE_OVERRIDE,kvantum` | Use Kvantum Qt style | Not set | NEW | `[x]` |
| `SDL_VIDEODRIVER,wayland` | Force SDL to Wayland | Not set | NEW | `[x]` |
| `MOZ_ENABLE_WAYLAND,1` | Firefox Wayland backend | Not set | NEW | `[x]` |
...
| `XDG_DATA_DIRS,$XDG_DATA_DIRS:$HOME/.nix-profile/share:...` | Extend XDG data path for .desktop files | Not set; Nix handles this via wrappers | NEW | `[x]` |
| `XCOMPOSEFILE,~/.XCompose` | XCompose input sequences file | Not set | NEW | `[x]` |
| `EDITOR,nvim` | Default text editor | Not set as env var (nvim is primary but not exported) | NEW | `[x]` |
| `GTK_THEME,Adwaita:dark` (or light) | GTK theme env var | Set via dconf in desktop.nix | PARTIAL | `[x]` |
| `xwayland.force_zero_scaling = true` | XWayland scaling override | Set in hyprland.lua | ALREADY PRESENT | — |
| `ecosystem.no_update_news = true` | Disable Hyprland update popup | Not set | NEW | `[x]` |
...
| `sensitivity = 0` | Mouse sensitivity unchanged | Not set explicitly | NEW | `[x]` |
| `touchpad.natural_scroll = false` | Disable natural scroll | `natural_scroll = true` in hyprland.lua | CONFLICT | `[x]` |
| `touchpad.disable_while_typing = true` | (implicit default) | `disable_while_typing = true` in hyprland.lua | ALREADY PRESENT | — |
| `gestures.workspace_swipe = false` | Disable touchpad swipe for workspaces | Not set | NEW | `[x]` |
...
| Setting | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `hexToRgba` helper function | Converts base16 palette hex → `rgba(r,g,b,a)` string | Not needed (uses `rgba(...)` literals) | NEW (if adopting nix-colors) | `[x]` |
| `general.col.active_border` | From `palette.base0D` + `palette.base0E` gradient | Hardcoded `rgba(00ffffee)` cyan | PARTIAL | `[x]` |
| `general.col.inactive_border` | From `palette.base03` | Hardcoded `rgba(595959aa)` gray | PARTIAL | `[x]` |
| `general.gaps_in = 5` | Inner window gap | Same (5) | ALREADY PRESENT | — |
| `general.gaps_out = 10` | Outer window gap | 0 (no outer gap) | PARTIAL | `[x]` |
...
| `decoration.rounding = 4` | Corner rounding in pixels | 8 in Technonomicon | PARTIAL | `[x]` |
| `decoration.blur.enabled = true` | Window blur | Not set (blur not configured) | NEW | `[x]` |
| `decoration.blur.size = 5` | Blur kernel size | — | NEW | `[x]` |
| `decoration.blur.passes = 2` | Blur quality passes | — | NEW | `[x]` |
| `decoration.shadow.enabled = false` | No window shadows | Not set | NEW | `[x]` |
...
| `bezierCurve "easeOutQuint"` | `0.23, 1, 0.32, 1` | No custom bezier curves | NEW | `[x]` |
| `bezierCurve "easeInOutCubic"` | `0.65, 0.05, 0.35, 0.95` | — | NEW | `[x]` |
| `bezierCurve "linear"` | `0, 0, 1, 1` | — | NEW | `[x]` |
| `bezierCurve "almostLinear"` | `0.5, 0.5, 0.75, 1.0` | — | NEW | `[x]` |
| `bezierCurve "quick"` | `0.15, 0, 0.1, 1` | — | NEW | `[x]` |
| `animation "global"` | `global, 1, 10, default` | No animations configured | NEW | `[x]` |
| `animation "border"` | `border, 1, 5.39, easeOutQuint` | — | NEW | `[x]` |
| `animation "windows"` | `windows, 1, 4.79, easeOutQuint` | — | NEW | `[x]` |
| `animation "windowsIn"` | `windowsIn, 1, 4.1, easeOutQuint, popin 87%` | — | NEW | `[x]` |
| `animation "windowsOut"` | `windowsOut, 1, 1.49, linear, popin 87%` | — | NEW | `[x]` |
| `animation "fadeIn"` | `fadeIn, 1, 1.73, almostLinear` | — | NEW | `[x]` |
| `animation "fadeOut"` | `fadeOut, 1, 1.46, almostLinear` | — | NEW | `[x]` |
| `animation "fade"` | `fade, 1, 3.03, quick` | — | NEW | `[x]` |
| `animation "layers"` | `layers, 1, 3.81, easeOutQuint` | — | NEW | `[x]` |
| `animation "layersIn"` | `layersIn, 1, 4, easeOutQuint, fade` | — | NEW | `[x]` |
| `animation "layersOut"` | `layersOut, 1, 1.5, linear, fade` | — | NEW | `[x]` |
| `animation "fadeLayersIn"` | `fadeLayersIn, 1, 1.79, almostLinear` | — | NEW | `[x]` |
| `animation "fadeLayersOut"` | `fadeLayersOut, 1, 1.39, almostLinear` | — | NEW | `[x]` |
| `animation "workspaces"` | `workspaces, 1, 1.94, almostLinear, fade` | — | NEW | `[x]` |
| `animation "workspacesIn"` | `workspacesIn, 1, 1.21, almostLinear, fade` | — | NEW | `[x]` |
| `animation "workspacesOut"` | `workspacesOut, 1, 1.94, almostLinear, fade` | — | NEW | `[x]` |
...
|---|---|---|---|---|
| `suppressevent maximize, class:.*` | Prevent all windows from requesting maximize | Not present | NEW | `[x]` |
| `tile, class:^(chromium)$` | Force Chromium to tile (workaround for `--app` mode) | Not needed (Brave doesn't have this bug) | SKIP | — |
| `float, class:^(org.pulseaudio.pavucontrol\|blueberry.py)$` | Float audio and bluetooth settings | Has `portal-dialog-size` float rule; no pavucontrol float rule | NEW | `[x]` |
| `float, class:^(steam)$` | Float Steam main window | Not present | NEW | `[x]` |
| `fullscreen, class:^(com.libretro.RetroArch)$` | Fullscreen RetroArch | Not present | NEW | `[x]` |
...
| `float, class:(clipse)` | Float clipse clipboard window | Not present (copyq used) | CONFLICT | `[x]` |
| `size 622 652, class:(clipse)` | Clipse window size | Not present | CONFLICT | `[x]` |
| `stayfocused, class:(clipse)` | Keep clipse focused | Not present | CONFLICT | `[x]` |
...
| `window-decoration = "none"` | No window title bar | Not set | NEW | `[x]` |
| `font-family = cfg.primary_font` | Font from omarchy config option | Not set (system monospace) | NEW | `[x]` |
| `font-size = 12` | Font size | Not set | NEW | `[x]` |
| `theme = "omarchy"` | Custom theme from base16 palette | Not set (no theme configured) | NEW (requires nix-colors) | `[x]` |
| `keybind = ["ctrl+k=reset"]` | Ctrl+K clears terminal | Not set | NEW | `[x]` |
...
| Setting | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `general.disable_loading_bar = true` | No loading bar | Not set | NEW | `[x]` |
| `general.no_fade_in = false` | Fade in on lock | Not set | NEW | `[x]` |
| `auth.fingerprint.enabled = true` | Fingerprint unlock support | Not present | NEW | `[ ]` |
| `background.path = selected_wallpaper_path` | Uses theme wallpaper as background | `background.color = rgba(00ffffee)` (solid cyan) | CONFLICT | `[x]` |
| `input-field.size = "600, 100"` | Large input field | `size = 300, 50` | PARTIAL | `[x]` |
| `input-field.position = "0, 0"` | Centered | `position = 0, -100` (slightly below center) | PARTIAL | `[x]` |
| `input-field.inner_color` | From `palette.base02` | Not set | NEW | `[x]` |
| `input-field.outer_color` | From `palette.base05` | Not set | NEW | `[x]` |
| `input-field.outline_thickness = 4` | Border thickness | Not set | NEW | `[x]` |
| `input-field.font_family = "CaskaydiaMono Nerd Font"` | Font | Not set | NEW | `[x]` |
| `input-field.font_size = 32` | Large font | Not set | NEW | `[x]` |
| `input-field.font_color` | From palette | Not set | NEW | `[x]` |
| `input-field.placeholder_text = "  Enter Password 󰈷 "` | Custom placeholder with icons | `placeholder_text = Password` | PARTIAL | `[x]` |
| `input-field.rounding = 0` | Square input field | Not set | NEW | `[x]` |
| `input-field.fade_on_empty = false` | Don't fade out when empty | Not set | NEW | `[x]` |
...
| Feature | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `home.file."Pictures/Wallpapers"` | Copies bundled wallpapers to ~/Pictures/Wallpapers | Not present | NEW | `[x]` |
| `services.hyprpaper.enable = true` | Enables hyprpaper wallpaper daemon | Not present | NEW | `[x]` |
| `services.hyprpaper.settings.preload` | Preloads selected wallpaper path | Not present | NEW | `[ ]` |
| `services.hyprpaper.settings.wallpaper` | Sets wallpaper per-monitor | Not present | NEW | `[x]` |
...
| Setting | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| `general.lock_cmd = "pidof hyprlock \|\| hyprlock"` | Prevent duplicate lock instances | `lock_cmd = hyprlock` (could spawn duplicates) | NEW | `[x]` |
| `general.before_sleep_cmd = "loginctl lock-session"` | Lock via loginctl on sleep | `before_sleep_cmd = hyprlock` (direct) | PARTIAL | `[x]` |
| `general.after_sleep_cmd = "hyprctl dispatch dpms on"` | Turn display on after wake | Same in Technonomicon | ALREADY PRESENT | — |
| `listener[0].timeout = 300` | Lock after 5 minutes idle | 600 seconds (10 minutes) and only on battery | PARTIAL | `[x]` |
| `listener[0].on-timeout = "loginctl lock-session"` | Trigger lock | `lock_cmd = hyprlock` | PARTIAL | `[x]` |
| `listener[1].timeout = 330` | Turn display off at 5:30 | No display-off listener in Technonomicon | NEW | `[x]` |
| `listener[1].on-timeout = "hyprctl dispatch dpms off"` | DPMS off | Not present | NEW | `[x]` |
| `listener[1].on-resume = "hyprctl dispatch dpms on && brightnessctl -r"` | Restore display and brightness | Technonomicon: `after_sleep_cmd = hyprctl dispatch dpms on` (no brightnessctl) | NEW | `[x]` |
...
| `settings.background-color` | From `palette.base00` | Hardcoded `rgba(0.04, 0.04, 0.08, 0.95)` | CONFLICT | `[x]` |
| `settings.text-color` | From `palette.base05` | Hardcoded `#cdd6f4` | CONFLICT | `[x]` |
| `settings.border-color` | From `palette.base04` | Hardcoded `#00ffff` (cyan) | CONFLICT | `[x]` |
| `settings.progress-color` | From `palette.base0D` | Not set | NEW | `[x]` |
| `settings.width = 420` | Notification width | 360px in quickshell | PARTIAL | `[x]` |
| `settings.height = 110` | Max notification height | Implicit in quickshell layout | PARTIAL | `[x]` |
| `settings.padding = "10"` | Inner padding | 10px margins in quickshell | PARTIAL | `[x]` |
| `settings.margin = "10"` | Outer margin | `margins.top: 44; margins.right: 8` | PARTIAL | `[x]` |
| `settings.border-size = 2` | Border width | 1px in quickshell | PARTIAL | `[x]` |
| `settings.border-radius = 0` | Square corners | `radius: 8` in quickshell | CONFLICT | `[x]` |
| `settings.anchor = "top-right"` | Position | `anchors { top: true; right: true }` (same) | ALREADY PRESENT | — |
| `settings.default-timeout = 5000` | 5 second auto-dismiss | Timer in quickshell (3000ms default, or `expireTimeout`) | PARTIAL | `[x]` |
| `settings.max-visible = 5` | Show up to 5 notifications | Unlimited in quickshell (all `trackedNotifications`) | PARTIAL | `[x]` |
| `settings.sort = "-time"` | Newest first | Quickshell uses list order | PARTIAL | `[x]` |
| `settings.group-by = "app-name"` | Group notifications by app | Not implemented in quickshell | NEW | `[x]` |
| `settings.format = "<b>%s</b>\\n%b"` | Bold summary + body | Separate Text elements in quickshell | ALREADY PRESENT | — |
| `settings.markup = true` | Allow HTML markup in notifications | Not set in quickshell | NEW | `[x]` |
...
| Bar height: 26px | 26px | 36px | PARTIAL | `[x]` |
| Background color | From `palette.base00` (via theme.css) | `Qt.rgba(0.04, 0.04, 0.08, 0.92)` hardcoded | PARTIAL | `[x]` |
...
| `bluetooth` module | Bluetooth icon, click → blueberry | Not present in quickshell bar | NEW | `[x]` |
| `network` module | WiFi/ethernet icon, tooltip bandwidth | WiFi/ethernet/offline icon, click → notify | PARTIAL | `[x]` |
| Network on-click | `ghostty -e nmcli` | Runs `/etc/scripts/net-info.sh` notify-send | PARTIAL | `[x]` |
...
|---|---|---|---|
| `font-family: CaskaydiaMono Nerd Font` | Bar font | NEW (requires that font) | `[x]` |
| `font-size: 14px` | Text size | NEW | `[x]` |
| Workspace button padding `2px 6px` | Compact buttons | NEW | `[x]` |
| `@import "./theme.css"` | Dynamic colors from nix-colors | NEW | `[x]` |
| Right-module `margin-right: 13px` | Spacing between widgets | NEW | `[x]` |
| `min-width: 12px` | Minimum widget size | NEW | `[x]` |
...
| Dimensions | 600×350px | 800px wide, positioned at 50% x 30% y | PARTIAL | `[x]` |
| Display mode | `drun` (desktop apps only) | libapplications + librink (math) + libshell (commands) | PARTIAL | `[x]` |
| Font | CaskaydiaMono Nerd Font 18px | Not configured (system default) | NEW | `[x]` |
| Window opacity | 0.95 | Not set | NEW | `[ ]` |
| Icon support | 40px images | `hide_icons: false` (default icons) | ALREADY PRESENT | — |
| Case-insensitive | Yes | Not configured | NEW | `[x]` |
| Markup | Yes | Not set | NEW | `[x]` |
| Colors | From `palette.base00/05/01/07` | Not configured (system theme) | NEW (requires nix-colors) | `[x]` |
...
| Setting | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| Custom btop theme file from base16 palette | Dynamic colors mapped from `palette.base0*` | No theme configured | NEW (requires nix-colors) | `[x]` |
| `color_theme` | Points to generated theme file | Not set | NEW | `[x]` |
| `rounded_corners = True` | Rounded TUI boxes | Not set | NEW | `[x]` |
| `graph_symbol = "braille"` | Braille characters for graphs | Not set | NEW | `[x]` |
| `vim_keys = True` | hjkl navigation | Not set | NEW | `[x]` |
| `shown_boxes = "cpu mem net proc"` | Show CPU, memory, network, processes | Not set | NEW | `[x]` |
| `update_ms = 2000` | 2 second refresh | Not set | NEW | `[x]` |
| `proc_sorting = "cpu lazy"` | Sort processes by CPU | Not set | NEW | `[x]` |
| `temp_scale = "celsius"` | Temperature in Celsius | Not set | NEW | `[x]` |
| `draw_clock = ""` | No clock in btop | Not set | NEW | `[x]` |
| (Theme) Main background: `palette.base00` | Terminal background color | — | NEW | `[x]` |
| (Theme) Main text: `palette.base05` | Primary text | — | NEW | `[x]` |
| (Theme) Title text: `palette.base0D` | Blue-ish accent | — | NEW | `[x]` |
| (Theme) CPU boxes: `palette.base0E` | Magenta accent | — | NEW | `[x]` |
| (Theme) Memory: `palette.base0B` | Green accent | — | NEW | `[x]` |
| (Theme) Network: `palette.base0C` | Cyan accent | — | NEW | `[x]` |
| (Theme) Processes: `palette.base0A` | Yellow accent | — | NEW | `[x]` |
...
| `extraConfig.credential.helper = "store"` | Store credentials in `~/.git-credentials` (plaintext) | No credential helper (uses SSH keys) | NEW | `[x]` |
| `programs.gh.enable = true` | GitHub CLI | Present in shell.nix packages | ALREADY PRESENT | — |
| `programs.gh.gitCredentialHelper.enable = true` | gh as git credential helper | Not set | NEW | `[x]` |
...
| `fonts.fontconfig.defaultFonts.serif = ["Noto Serif]` | System serif font | Not set | NEW | `[x]` |
| `fonts.fontconfig.defaultFonts.sansSerif = ["Noto Sans"]` | System sans-serif font | Not set | NEW | `[x]` |
| `fonts.fontconfig.defaultFonts.monospace = ["Caskaydia Mono Nerd Font"]` | System monospace font | Not set (JetBrains Mono installed but not set as default) | NEW | `[x]` |
...
| Feature | omarchy-nix | Technonomicon | Status | Import? |
|---|---|---|---|---|
| Bash script to parse `~/.config/hypr/hyprland.conf` for bindings | Reads `bind =` and `bind=` lines, extracts modifier+key | Not present | NEW | `[x]` |
| Handles both `bind =` and `bind=` formats | Regex-based parsing | — | NEW | `[x]` |
| Displays results in wofi at 50% width / 40% height | Shows filterable list of all keybindings | Not present | NEW | `[x]` |
| Uses `flock` to prevent multiple instances | Single-instance guarantee | — | NEW | `[x]` |
